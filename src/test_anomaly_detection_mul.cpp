...
int main(void){
  ...
  cv::Mat offset_mat = (cv::Mat_<double>(2, 1) << 170.0, 63.6);
  ...
  // }
  cv::Mat abnormal_a = (cv::Mat_<double>(2, 1) << 65.5, 170.0);
  cv::Mat abnormal_b = (cv::Mat_<double>(2, 1) << 715.8, 78.2);
  cv::Mat abnormal_c = (cv::Mat_<double>(2, 1) << 110.0, 32.5);
  normal_data.push_back(abnormal_a);
  normal_data.push_back(abnormal_b);
  normal_data.push_back(abnormal_c);

  // Set anomary threshold (The significance level is set 0.01% in this case)
  double a_th = ChisqrSetCriticalValue(1, 0.001);
  cout << "a_th: " << a_th << endl;
